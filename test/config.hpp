...
mrb_value spec_error_assets_in(mrb_state *mrb, mrb_value)
{
    const char *text = nullptr;
    mrb_int length = 0;
    mrb_get_args(mrb, "s", &text, &length);
    const auto path = config::error_assets_in(mrb, std::string_view(text, static_cast<size_t>(length)));
    if (!path)
        return mrb_str_new_cstr(mrb, path.error().what());
    if (!*path)
        return mrb_nil_value();
    return mrb_symbol_value(mrb_intern(mrb, (*path)->data(), (*path)->size()));
}
...
inline void config_spec(mrb_state *mrb)
{
    ...
    mrb_define_module_function(mrb, sp, "error_assets_in", spec_error_assets_in, MRB_ARGS_REQ(1));
}
