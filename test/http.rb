...
assert('next_path_segment walks a path one path segment at a time') do
  walk = Webmachine::SpecHttp.next_path_segment('/a/b/c')
  assert_equal 'a', walk[WALK_PATH_SEGMENT]
  assert_equal '/b/c', walk[WALK_REST]
  walk = Webmachine::SpecHttp.next_path_segment('/c')
  assert_equal 'c', walk[WALK_PATH_SEGMENT]
  assert_equal '', walk[WALK_REST]
  walk = Webmachine::SpecHttp.next_path_segment('/')
  assert_equal '', walk[WALK_PATH_SEGMENT]
  assert_equal '', walk[WALK_REST]
end
...
def walk_path(path)
  ...
  rest = path
  until rest.empty?
    walk = Webmachine::SpecHttp.next_path_segment(rest)
    out << walk[WALK_PATH_SEGMENT]
    rest = walk[WALK_REST]
  end
  out
end

assert('next_path_segment gives a trailing slash its empty path segment') do
