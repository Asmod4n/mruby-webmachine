...
task :bench do
  ...
  filter = ENV.fetch('WM_FILTER', '')
  chosen = filter.empty? ? [] : ["--benchmark_filter=#{filter}"]
  sh(*bench_runner, binary, *chosen, '--benchmark_repetitions=5',
...
# kErrorAssetsPrefix in src/problem.hpp: where the server mounts this
# pack, and therefore the src of every picture in it.
def error_assets_prefix
  File.read(File.expand_path('src/problem.hpp', __dir__))[
    /kErrorAssetsPrefix = "([^"]*)"/, 1] or
    raise 'src/problem.hpp: no kErrorAssetsPrefix to build the picture URLs from'
end

# RFC 9110 15: what the server calls each error, read out of
# http::reason_phrase in src/http.hpp rather than copied to a second place
# that can drift. A status it has no phrase for gets a picture whose alt
# names the number alone.
def status_titles
  out = {}
  reason = File.read(File.expand_path('src/http.hpp', __dir__))[
    /constexpr std::string_view reason_phrase\(const uint16_t status\)\n\{(.*?)\n\}/m, 1] or
    raise 'src/http.hpp: no reason_phrase() to read the status names from'
  reason.scan(/case (\d+):\s*return "([^"]*)";/) { |c, n| out[c.to_i] = n if c.to_i >= 400 }
  raise 'no status names found' if out.empty?
  out
end

ERROR_NOTICE = <<~TEXT
  mruby-webmachine error pages
  ============================
  ...
  This pack holds PICTURES. The pages themselves live in the server, in
  src/problem.hpp - a server with no pack still has to be able
...
task :error_assets do
  ...
  ERROR_CODES.each do |code|
    known = have[code]
    headers = { 'User-Agent' => 'mruby-webmachine error-assets packer', read_timeout: 20 }
  ...
  # src/problem.hpp; the licence travels in the pack's own comment.
...
# PREFIX/share/mruby-webmachine, and its configuration in its own
# directory under PREFIX/etc. The configuration is the operator's file:
# install writes one only where there is none, with the path of the
# pack it just put down.
INSTALL_PROGRAMS = %w[webmachine-serve webmachine-cache webmachine-cache-check].freeze
RELEASE_BIN = File.expand_path('mruby/build/release/bin', __dir__)

def install_dirs(prefix)
  { bin: File.join(prefix, 'bin'),
    share: File.join(prefix, 'share', 'mruby-webmachine'),
    etc: File.join(prefix, 'etc', 'webmachine') }
end

desc 'install the release build: rake install[PREFIX], PREFIX is /usr/local'
task :install, %i[prefix] do |_t, args|
  dirs = install_dirs(args[:prefix] || '/usr/local')
  missing = INSTALL_PROGRAMS.reject { |p| File.executable?(File.join(RELEASE_BIN, p)) }
  raise "no release build at #{RELEASE_BIN}: #{missing.join(', ')} - build with MRUBY_CONFIG=build_config_release.rb" unless missing.empty?

  dirs.each_value { |d| mkdir_p d }
  INSTALL_PROGRAMS.each { |p| install File.join(RELEASE_BIN, p), dirs[:bin], mode: 0o755 }
  archive = File.join(dirs[:share], 'error-assets.zip')
  install ERROR_ASSETS, archive, mode: 0o644
  conf = File.join(dirs[:etc], 'webmachine.toml')
  if File.exist?(conf)
    puts "#{conf} is there and is kept"
  else
    example = File.read(File.expand_path('webmachine.toml.example', __dir__))
    File.write(conf, example.sub(/^# error_assets = .*$/, "error_assets = #{archive.inspect}"))
    chmod 0o644, conf
  end
end

# The other half of install. The configuration is the operator's own
# file, so it stays, and its directory with it.
desc 'remove what rake install put down: rake uninstall[PREFIX], PREFIX is /usr/local'
task :uninstall, %i[prefix] do |_t, args|
  dirs = install_dirs(args[:prefix] || '/usr/local')
  INSTALL_PROGRAMS.each do |p|
    path = File.join(dirs[:bin], p)
  ...
  rmdir dirs[:share] if File.directory?(dirs[:share]) && Dir.empty?(dirs[:share])
  conf = File.join(dirs[:etc], 'webmachine.toml')
  puts "#{conf} is yours and is kept - remove it by hand to start over" if File.exist?(conf)
end
